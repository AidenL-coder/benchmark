...
\title{Zero Actions: A Published Self-Improving Agent Scaffold Takes\\
No Tool Calls at 7B and 14B Scale---and Pass Rate Cannot Tell}
...
\begin{abstract}
We run the unmodified default agent of a published, award-recognised
self-improving coding system (HGM) on \emph{its own} 59-task Polyglot
baseline, with a frozen \texttt{Qwen2.5-Coder-7B-Instruct}, and observe
\textbf{zero}: 0/59 tasks resolved, 0 tool calls, 59/59 empty patches,
uniform across six languages with no exception. The model is not failing at
code: it emits a mean of 902 tokens of competent analysis per task, then
stops without editing a file, in one case politely instructing the user to
apply its patch by hand. \textbf{Doubling model scale does not help}:
\texttt{Qwen2.5-Coder-14B} reproduces the 7B result in every cell, falsifying
a prediction we registered in advance. What does help is a two-line harness
edit (\texttt{tool\_choice} \texttt{"auto"}$\to$\texttt{"required"}), which
moves the same model to 59/59 tasks using tools and 3663 tool calls
($p = 3.4\times10^{-13}$).

The consequence for meta-agent evaluation is the point of this paper. These
two behaviourally opposite configurations are \emph{statistically
indistinguishable in pass rate} ($2/59$ vs $0/59$, $p = 0.50$) despite
differing $62\times$ in consumed compute. Pass rate, the signal DGM, HGM,
and SICA all select on, cannot see whether the agent is an agent, and the
resulting all-zero utility is mechanically why the forked search crashes
before its first expansion step. Forcing tool use also does not confer
task competence: in 9/59 cases the agent worked hard but wrote a file of its
own naming, often in the wrong language entirely, leaving nothing gradeable.
We therefore argue that reported self-improvement gains are currently
unattributable, and contribute the instrument the attribution requires:
matched-compute elicitation controls, operation tagging enforced by
\emph{network-layer interception} rather than agent self-report, causal
ablation that withholds a capability rather than requesting its disuse, and
...
\begin{enumerate}
\item A \emph{published, award-recognised} system's own default agent produces
  \textbf{no actions whatsoever} on \emph{its own} benchmark at 7B and 14B
  scale. Not degraded performance: zero tool calls, zero non-empty patches,
  59/59 tasks, six languages.
\item \textbf{Scale does not rescue it.} We registered the opposite prediction
  in advance and report it falsified.
\item \textbf{Pass rate cannot detect any of this.} Two configurations
  differing totally in behaviour and $62\times$ in compute are statistically
  indistinguishable in the very signal used to select between
  scaffolds---so a search optimising it is blind to the property that
  determines whether its agent acts at all.
\end{enumerate}

Section~\ref{sec:instrument} describes the instrument that makes such
attribution measurable, designed to attach to an existing,
independently-implemented loop rather than to a system co-designed with its
own evaluation.
...
\caption{The same two-line scaffold change, measured two ways, on a shared
$0$--$59$ axis. Left: whether the agent takes any action at all. Right:
whether it solves the task. Both model scales behave identically under the
default policy (both at zero, left panel), so the separation is produced by
the tool policy rather than by capability. Plotting both panels on one scale
is deliberate: on its own axis the right-hand panel would look like a large
effect, which is the misreading this paper argues against.}
\label{fig:asymmetry}
\end{figure}}{}
...
definitional. Three things about \emph{how} it
lands are not definitional, and are what we report: the baseline it moves from
is exactly zero rather than merely low; the same flag is what separates ``this
system does nothing'' from ``this system solves tasks'' on a benchmark the
system's authors chose; and, as the implications below show, the
change is invisible to pass rate. A flag whose effect is definitional at the
behavioural level can still be undetectable at the level a search actually
optimises---that gap is the finding.
...
scope the $2\times2$ separates cleanly. Tool policy predicts agentic
...
\paragraph{Magnitude of the interaction term.}
...
\paragraph{Why moving off zero is algorithmically decisive.}
