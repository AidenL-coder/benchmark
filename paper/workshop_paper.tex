...
14B $\times$ \texttt{required} & \todoresult{n} & \todoresult{res} & \todoresult{tools} & \todoresult{calls} & \todoresult{gens} \\
\bottomrule
\end{tabular}
\end{table}
...
\todoresult{7B x required narrative, tool call counts, resolve rate, and the
comparison against the auto arm}
...
\todoresult{quantified statement once required-arm numbers exist}
A scaffold search that happens to discover a tool-policy change---or any
equivalently trivial harness adjustment---would register as a large capability
gain under current evaluation practice, while the frozen model's reachable set
...
most.}
Combining the two: the arms differ by $p\approx10^{-13}$ in behaviour and are
indistinguishable in the signal used to select between them. A search procedure
optimising pass rate---as DGM, HGM, and SICA all do---receives almost no
gradient from the single change that determines whether its agent is an agent.
Whatever such a search discovers, it does not discover it by seeing this.

This is not hypothetical. HGM's node-selection step estimates each archive
entry's promise from its measured utility and expands the most promising. We
found that its implementation raises \texttt{ValueError: attempt to get argmax
of an empty sequence} whenever every archive node has utility exactly zero,
because the candidate set is filtered to nodes with strictly positive utility
before sampling. In the regime characterised here that filter is \emph{always}
empty, so the degenerate branch is not an edge case but the expected path. An
evolutionary search cannot bootstrap from a uniformly-zero signal, and the
scaffold change that would break the deadlock is precisely the one its
objective cannot see.

\paragraph{The scaffold, not the model, decides whether an agent acts.}
\todoresult{final 2x2} Across \todoresult{n} tasks and two model scales, tool
policy predicts agentic behaviour essentially perfectly while model scale
predicts it not at all. For meta-agent evaluation this reverses the usual
attribution: a harness search that stumbles onto a tool-policy change is not
discovering a better agent, it is discovering the switch that makes the agent
an agent---and it will be credited with a capability gain either way.

The practical implication is that self-improvement results are only
interpretable within a band: the base model must be strong enough that the
selection signal is non-degenerate, but the scaffold must already be
functional enough that the search is not merely rediscovering its own
switches. Neither boundary is currently reported by any system we reviewed.
...
\todoresult{one or two} sizes, one task family, and one forked system. Whether
